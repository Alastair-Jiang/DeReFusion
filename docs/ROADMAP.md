...
**已完成（v2，2026-09-11 23:20）**
- `gatev1-volatilityaware`（GSPC T=24）：**MSE 0.06767** —— 优于线性（0.07007），但**劣于无参数加法**（0.06230，差 8.6%）。
- BTCUSD 分层（n=707）：全样本仅领先 2.4%；**交互效应 +0.01502（显著正）**，与 GSPC 的 −0.01205（显著负）**符号相反**。
- **ETHUSD（n=573）**：交互效应 −0.00943，95%CI[−0.01881,+0.00033]（方向同 GSPC、统计临界）。
- **GSPC seed 2022**：交互效应 **−0.02248**（显著负）→ GSPC 结论**跨种子稳健**。

**核心结论**
- 3 资产中 2 个同向（GSPC 强/稳健、ETH 弱/临界）、1 个反向（BTC 显著）→ **资产依赖、非普适**。
- 唯一跨资产一致现象：平静期两分支打平。
- 决策：**自适应融合止损；NS spike 暂缓**。依据：①加法在 3 资产全优 ②门控实测劣于加法（无切换空间）③结构成立处也未能转化为收益。
- 结构感知路由主线（方向 A）不受影响。

**下一步（按信息量）**
- 补齐 7 个资产测**交互效应符号分布**（纯 CPU、零 token）→ 决定该方向彻底关闭还是存在资产级规律。
- GSPC seed 2023 与 ETH 多种子（钉死临界结论）。
- 精力回归结构感知路由主线。
