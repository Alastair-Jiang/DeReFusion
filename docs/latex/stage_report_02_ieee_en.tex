...
\begin{tabular}{@{}p{0.07\columnwidth}p{0.55\columnwidth}p{0.24\columnwidth}@{}}
\toprule
Stage & Content & Status \\
\midrule
S1 & Reproduction and controlled diagnostics & complete \\
S2 & Structural validation (candidate regularity) & \textbf{FAIL} \\
...
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{0.26\columnwidth}p{0.20\columnwidth}p{0.40\columnwidth}@{}}
\toprule
Property & Value & Note \\
\midrule
Assets & 20 & pre-named universe \\
Provider & Yahoo & single source \\
Window & 2016-01-04/01 -- 2025-12-31 & daily OHLC, unadjusted \\
Exclusions & 0 & all 20 accepted \\
Rows per asset & 2444--2602 & exchange calendars \\
...
Concurrency & 2 lanes & widening declined \\
Threads & 14 & unchanged mid-campaign \\
Retained & metrics, pred, true & + command line, log tail \\
Checkpoints & not transferred & not needed \\
Batch reporting & 40 runs / 6 h & manifest per batch \\
Progress & 0/120 & launched 13:36 \\
\bottomrule
\end{tabular}
...
mechanisms: idempotent skipping; a watchdog that restarts lanes without ever
duplicating a run; stall detection by \emph{process-tree CPU time} rather than log output;
and a contingency plan that pre-authorises self-recovery while forbidding any protocol or data
change. One genuine defect was found and fixed: a restart orphaned two
finished runs that no lane recorded, which would have left a silent two-run gap; the runner now
ingests such runs and flags them in the manifest, so the process, not merely the content, is
auditable.
...
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{0.07\columnwidth}p{0.51\columnwidth}p{0.28\columnwidth}@{}}
\toprule
\# & Milestone & Status \\
\midrule
M1 & Cohort acquired (peer machine) & done \\
M2 & Intake validation & done, 20/20 \\
M3 & Cohort locked (hash table) & done \\
M4 & Predictor frozen (leak check) & done \\
M5 & Verifier hardened, smoke-tested & done \\
M6 & Panel launched (120 runs) & running, 0/120 \\
M7 & First batch verified & pending \\
